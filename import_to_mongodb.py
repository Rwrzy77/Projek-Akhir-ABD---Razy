"""
Import Merged IT Jobs Dataset ke MongoDB
=========================================
Script ini membaca merged_it_jobs.csv dan mengimportnya ke MongoDB.
Database: bigdata_jobs
Collection: it_jobs
"""
...
def clean_value(val):
    """Bersihkan nilai NaN/None menjadi None untuk MongoDB"""
    if pd.isna(val):
        return None
...
def prepare_documents(df):
    """Konversi DataFrame ke list of MongoDB documents"""
    documents = []
    ...
    for _, row in df.iterrows():
        ...
        # Hitung rata-rata gaji jika keduanya tersedia
...
def create_indexes(collection):
    """Buat index untuk query yang efisien"""
    print("\n📋 Membuat indexes...")
    ...
    # Compound index untuk query gabungan
    collection.create_index([
        ("platform", ASCENDING),
        ("country", ASCENDING),
        ("keyword", ASCENDING)
...
def print_summary(collection):
    """Tampilkan ringkasan data di MongoDB"""
    print("\n" + "=" * 60)
...
def main():
    ...
    if old_count > 0:
        print(f"\n🗑️  Menghapus {old_count:,} dokumen lama...")
    ...
    # Batch insert (500 dokumen per batch)
    batch_size = 500
    total_inserted = 0
